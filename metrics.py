    ...
    for i in range(0, user_emb.shape[0], batch):
        ...
        for j in range(batch):
            if i + j >= user_emb.shape[0]:
                break
            mask[j].scatter_(dim=0, index=torch.tensor(list(train_user_list[i + j])).to(args.device),
                             value=torch.tensor(0.0).to(args.device))
    ...
    for i in range(0, user_emb.shape[0], batch):
        ...
        for j in range(batch):
            if i + j >= user_emb.shape[0]:
                break
            try:
                mask[j].scatter_(dim=0, index=torch.LongTensor(list(train_user_list[i + j])).to(args.device),
                                 value=torch.tensor(0.0).to(args.device))
            except:
                pass
        # Calculate prediction value
...
def build_ndcg_k_list(user_emb, item_emb, train_user_list, test_user_list, topk, batch=200):
    ...
    for i in range(0, user_emb.shape[0], batch):
        ...
        for j in range(batch):
            if i + j >= user_emb.shape[0]:
                break
            mask[j].scatter_(dim=0, index=torch.LongTensor(list(train_user_list[i + j])).cuda(),
